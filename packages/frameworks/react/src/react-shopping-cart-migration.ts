...
export function planShoppingCartVite8Migration(input: {
	...
	if (
		...
		throw new Error('Shopping Cart tree-derived package identity differs');
	...
	if (
		...
		throw new Error('Shopping Cart bounded Vite 8 migration differs');
	const packageJson = `${JSON.stringify({ ...value, devDependencies: { ...value.devDependencies, vite: '8.0.16' } }, null, 2)}\n`;
	const body = {
		packageJson,
