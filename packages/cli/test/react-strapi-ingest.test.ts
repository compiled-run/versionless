...
import {
	...
	discoverStrapiApplicationPackage,
	isCanonicalMitLicense,
	parseStrapiJsonc,
	parseStrapiLauncher,
} from '../src/fixture/react-strapi-ingest.ts';
...
describe('Strapi T687 immutable ingest', () => {
	it('durably persists a response body and sanitized journal row before returning', async () => {
		const destination = await mkdtemp(join(tmpdir(), 'versionless-strapi-ledger-'));
	...
	it('accepts BOM, comments, and trailing commas without weakening JSON', () => {
		expect(
			parseStrapiJsonc('\uFEFF{/*x*/"name":"ui",// y\n"items":[1,],}', 'ui/package.json'),
		).toEqual({ name: 'ui', items: [1] });
	});
	it('rejects duplicate keys and invalid JSONC with a relative path class', () => {
		expect(() => parseStrapiJsonc('{"name":1,"name":2}', 'ui/package.json')).toThrow(
			'ui/package.json',
		);
		expect(() => parseStrapiJsonc('{"name":Infinity}', 'ui/package.json')).toThrow(
			'invalid-jsonc',
		);
	});
	it('accepts canonical MIT text semantically', () => {
		expect(
			isCanonicalMitLicense(
				'MIT License\nPermission is hereby granted, free of charge, to any person obtaining a copy\nTHE SOFTWARE IS PROVIDED "AS IS"',
			),
		).toBe(true);
		expect(isCanonicalMitLicense('MIT')).toBe(false);
	});
	...
	it('requires fresh literal consent', () => {
		...
		expect(parseStrapiLauncher(['--consent-id', STRAPI_CONSENT, '--namespace', 't687'])).toBe(
			'acquire',
		);
	});
	it('allows strict offline replay', () => {
		...
		expect(parseStrapiLauncher(['--verify-offline', '--namespace', 't687'])).toBe('verify');
	});
});
