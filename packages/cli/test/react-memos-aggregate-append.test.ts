...
import {
	appendReactMemosAggregateMembers,
	reactMemosAggregateMembers,
} from '../src/fixture/react-memos-aggregate-append.ts';
import {
...
const evidenceFiles = [
	WITNESS_REACT_MEMOS_RECEIPT_PATH,
	`${path.dirname(WITNESS_REACT_MEMOS_RECEIPT_PATH)}/receipt.md`,
	REACT_MEMOS_BUILD_LANES_PATH,
 ...
 * against its real jira-clone browser-proof predecessor now that the published
 * aggregate already carries the memos member with the killedbygoogle v3 and
 * LinkFree members appended on top of it.
 */
async function stagedRoot(): Promise<string> {
	const directory = await mkdtemp(path.join(os.tmpdir(), 'memos-aggregate-'));
...
describe('React memos aggregate append', () => {
	it('derives its single member from the published evidence rather than an authored row', async () => {
		const members = await reactMemosAggregateMembers(repositoryRoot);
	...
	it('appends the exact member and re-derives the browser-proof state', async () => {
		...
		try {
			...
			const result = await appendReactMemosAggregateMembers(directory);
			...
			expect(appended).toHaveLength(23);
			expect(appended.at(-1)?.receipt).toBe(WITNESS_REACT_MEMOS_RECEIPT_PATH);
			...
			await expect(appendReactMemosAggregateMembers(directory)).resolves.toEqual({
				kind: 'react-memos-browser-proof',
				receipts: 23,
				appended: false,
			});
	...
	it('refuses a predecessor that is not the jira-clone browser proof', async () => {
		const directory = await stagedRoot();
		try {
			await rewrite(directory, (members) =>
				members.filter(
					(member) => member.receipt !== WITNESS_ANGULAR_JIRA_CLONE_RECEIPT_PATH,
				),
			);
			expect(deriveCorpusTransactionState(await fixtures(directory)).kind).toBe(
				'angular-factoriolab-browser-proof',
			);
			await expect(appendReactMemosAggregateMembers(directory)).rejects.toThrow(
				/jira-clone browser-proof predecessor/,
			);
	...
	it('refuses a member appended anywhere but the tail', async () => {
		const directory = await stagedRoot();
		try {
			const members = await reactMemosAggregateMembers(directory);
			...
			await expect(appendReactMemosAggregateMembers(directory)).rejects.toThrow(
				/React memos browser proof aggregate receipt order differs/,
			);
			expect(await fixtures(directory)).toHaveLength(23);
		} finally {
			await rm(directory, { recursive: true, force: true });
	...
	it('refuses a member whose row drifts from the derived one', async () => {
		...
		try {
			...
			await rewrite(directory, (current) => [
				...current,
				{ ...members.witness, framework: 'angular' },
			]);
			await expect(appendReactMemosAggregateMembers(directory)).rejects.toThrow(
				/React memos aggregate membership/,
			);
		} finally {
			await rm(directory, { recursive: true, force: true });
		}
