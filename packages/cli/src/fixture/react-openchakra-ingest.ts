...
import {
	...
	type YarnArtifact,
} from './react-excalidraw-v011-ingest.ts';

export const OPENCHAKRA_CONSENT =
	'T564-official-source-openchakra-legacy-react-production-pilot' as const;
const hex40 = createRegExp(
	charIn('0123456789').from('a', 'f').times(40).at.lineStart().at.lineEnd(),
);
const root = resolve(import.meta.dirname, '../../../..');
const fixturePath = join(root, 'fixtures/react-openchakra/fixture.json');
const cacheRoot = join(root, '.versionless/cache/react-openchakra');
const stageRoot = join(root, '.versionless/stage/react-openchakra');
const workRoot = join(root, '.versionless/work/react-openchakra');
const runRoot = join(root, 'evidence/runs/react-openchakra');
const evidenceRoot = join(root, 'evidence/ingests/react-openchakra');
const dependenciesRoot = join(root, 'evidence/dependencies/react-openchakra');
...
type State = { responses: number; bytes: number; ledger: LedgerRow[] };
type Candidate = {
	commit: string;
	tree: string;
	treeRows: TreeRow[];
...
export function assertOpenChakraConsent(args: string[]): void {
	if (
		args.length !== 3 ||
		args[0] !== '--acquire' ||
		args[1] !== '--consent-id' ||
		args[2] !== OPENCHAKRA_CONSENT ||
		process.env.VERSIONLESS_NETWORK_MODE !== 'consented' ||
		process.env.VERSIONLESS_CONSENT_ID !== OPENCHAKRA_CONSENT
	)
		throw new Error('OpenChakra acquisition requires exact one-shot consent');
}

export function assertOpenChakraUrl(url: string, allowed: ReadonlySet<string>): void {
	...
	if (
		...
		throw new Error('OpenChakra URL is outside exact consent');
}

export function decodeOpenChakraBlob(
	value: unknown,
	expected: { sha: string; size: number },
): Buffer {
	if (!value || typeof value !== 'object' || Array.isArray(value))
		throw new Error('OpenChakra Git blob response differs');
	...
	if (
		...
		throw new Error('OpenChakra Git blob identity differs');
	const bytes = Buffer.from(row.content, 'base64');
	if (bytes.length !== expected.size || gitBlobSha(bytes) !== expected.sha)
		throw new Error('OpenChakra reconstructed Git blob differs');
	return bytes;
}
...
function verifySri(bytes: Buffer, integrity: string): void {
	if (createHash('sha512').update(bytes).digest('base64') !== integrity.slice('sha512-'.length))
		throw new Error('OpenChakra artifact integrity differs');
}
...
async function filesBelow(directory: string): Promise<string[]> {
	...
	for (const entry of await readdir(directory, { withFileTypes: true })) {
		...
		else throw new Error('OpenChakra source contains a special filesystem entry');
	}
...
async function getExact(
	...
	assertOpenChakraUrl(url, allowed);
	if (state.responses >= maxResponses) throw new Error('OpenChakra response cap exceeded');
		...
		const call = request(
			...
			(response) => {
				if (
					...
					response.resume();
					reject(new Error('OpenChakra response boundary differs'));
					return;
				}
				const chunks: Buffer[] = [];
				let bytes = 0;
				response.on('data', (chunk: Buffer) => {
					...
					if (state.bytes + bytes > maxBytes)
						call.destroy(new Error('OpenChakra byte cap exceeded'));
...
async function getBlob(
	...
	if (row.type !== 'blob' || row.size === undefined || !hex40.test(row.sha))
		throw new Error('OpenChakra tree blob identity differs');
	const url = joinURL(apiRoot, 'blobs', row.sha);
	allowed.add(url);
	return decodeOpenChakraBlob(
...
async function qualifyCandidate(
	...
	if (commit.sha !== commitSha || !hex40.test(commit.tree?.sha ?? ''))
		throw new Error('OpenChakra commit identity differs');
	...
	if (tree.sha !== commit.tree!.sha || tree.truncated !== false || !Array.isArray(tree.tree))
		throw new Error('OpenChakra tree identity differs');
	...
	if (!packageRow || !licenseRow || !lockRow) return null;
	...
	if (
		...
		return null;
	try {
		analyzeExcalidrawYarnLock(lockBytes);
	} catch {
		return null;
	}
	...
	if (retainedNotices.some((row) => row.expression === 'unknown')) return null;
	return {
		commit: commitSha,
		tree: tree.sha!,
		treeRows: tree.tree,
...
async function sealFailure(message: string): Promise<void> {
	...
	const receipt = {
		schemaVersion: 'versionless.react-openchakra-consumed-failed.v1',
		result: 'consumed-failed',
		consentId: OPENCHAKRA_CONSENT,
		retry: false,
		reusable: false,
...
export async function verifyOpenChakraIngest(): Promise<{ valid: true; digest: string }> {
	...
	if (sha256(canonicalize(copy)) !== receipt.integrity?.canonicalDigest)
		throw new Error('OpenChakra receipt digest differs');
	...
	if (sha256(canonicalize(manifest)) !== receipt.source?.manifestDigest)
		throw new Error('OpenChakra source replay differs');
	...
	if (sha256(canonicalize(receipt.closure?.artifacts)) !== receipt.closure?.digest)
		throw new Error('OpenChakra closure replay differs');
	return { valid: true, digest: receipt.integrity!.canonicalDigest! };
}

export async function acquireOpenChakra(): Promise<void> {
	for (const target of [
		cacheRoot,
		stageRoot,
		workRoot,
		...
		if (await exists(target)) throw new Error('OpenChakra acquisition requires fresh roots');
	await mkdir(evidenceRoot, { recursive: true });
	await writeFile(
		attemptPath,
		`${canonicalize({ schemaVersion: 'versionless.react-openchakra-attempt.v1', consentId: OPENCHAKRA_CONSENT, invoked: true })}\n`,
		{ flag: 'wx' },
	);
	...
	if (
		...
		repository.full_name !== 'premieroctet/openchakra' ||
		repository.html_url !== fixture.repository
	)
		throw new Error('OpenChakra canonical repository identity differs');
	...
	if (
		...
		throw new Error('OpenChakra candidate revision boundary differs');
	const apiRoot = joinURL('https://api.github.com', 'repos', 'premieroctet', 'openchakra', 'git');
	let selected: Candidate | null = null;
	for (const row of candidates) {
		selected = await qualifyCandidate(row.sha!, apiRoot, allowed, state);
		if (selected) break;
	}
	if (!selected)
		throw new Error('No qualifying OpenChakra revision exists within two official revisions');
	...
	const treeByPath = new Map(
		selected.treeRows.filter((row) => row.type === 'blob').map((row) => [row.path, row]),
	);
	const manifest = await Promise.all(
		(await filesBelow(source)).map(async (file) => {
			const bytes = await readFile(file);
			...
			if (!expected || expected.size !== bytes.length || expected.sha !== gitBlobSha(bytes))
				throw new Error(`OpenChakra archive/tree identity differs: ${path}`);
			return { path, bytes: bytes.length, sha256: sha256(bytes), gitSha: expected.sha };
		}),
	);
	if (manifest.length !== treeByPath.size)
		throw new Error('OpenChakra complete source manifest differs');
	if (
		!(await readFile(join(source, 'package.json'))).equals(selected.packageBytes) ||
		!(await readFile(join(source, selected.lockPath))).equals(selected.lockBytes)
	)
		throw new Error('OpenChakra archive/blob equality differs');
	const closure = analyzeExcalidrawYarnLock(selected.lockBytes);
	if (closure.artifacts.length > maxResponses - state.responses - 2)
		throw new Error('OpenChakra response cap cannot contain closure');
	...
	if (
		...
		throw new Error('OpenChakra Yarn tool identity differs');
	...
	if (
		...
		throw new Error('OpenChakra established runtime identity differs');
	...
	const receipt = {
		schemaVersion: 'versionless.react-openchakra-ingest.v1',
		result: 'pass',
		consentId: OPENCHAKRA_CONSENT,
		...
		nonclaims: [
			'not certification',
			'historical runtime not yet executed',
	...
	await writeFile(
		...
		`${canonicalize({ schemaVersion: 'versionless.react-openchakra-dependencies.v1', closure, audit, sourceReceipt: receipt.integrity.canonicalDigest })}\n`,
		{ flag: 'wx' },
	);
	const first = await verifyOpenChakraIngest();
	const second = await verifyOpenChakraIngest();
	if (first.digest !== second.digest) throw new Error('OpenChakra offline replay differs');
}

export async function main(args = process.argv.slice(2)): Promise<void> {
	assertOpenChakraConsent(args);
	try {
		await acquireOpenChakra();
		process.stdout.write(
			`${canonicalize({ result: 'pass', consentId: OPENCHAKRA_CONSENT })}\n`,
		);
	} catch (error) {
		await rm(stageRoot, { recursive: true, force: true });
...
if (process.argv[1]?.endsWith('react-openchakra-ingest.ts'))
