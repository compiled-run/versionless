...
export const STRAPI_CONSENT = 'T687-strapi-v3.6.8-production-acquisition' as const;
export const STRAPI_NAMESPACE = 't687' as const;

const root = resolve(import.meta.dirname, '../../../..');
const evidenceRoot = join(root, 'evidence/ingests/react-strapi/t687');
...
const stageRoot = join(root, '.versionless/cache/react-strapi-stage-t687');
const cacheRoot = join(root, '.versionless/cache/react-strapi-source-t687');
...
export type StrapiNetworkState = {
...
export function parseStrapiLauncher(args: readonly string[]): 'acquire' | 'verify' | 'preflight' {
	if (args.at(-2) !== '--namespace' || args.at(-1) !== STRAPI_NAMESPACE)
		throw new Error('Strapi namespace differs');
	...
	if (
		...
		leading[1] === STRAPI_CONSENT &&
		process.env.VERSIONLESS_NETWORK_MODE === 'consented' &&
		process.env.VERSIONLESS_CONSENT_ID === STRAPI_CONSENT
	)
		return 'acquire';
	if (
		leading.length === 1 &&
		...
		return 'verify';
	throw new Error('Strapi launcher boundary differs');
}

export function parseStrapiJsonc(text: string, path: string): unknown {
...
export type StrapiPackageCandidate = Readonly<{
	path: string;
	name: string | null;
...
export function isCanonicalMitLicense(text: string): boolean {
...
export function discoverStrapiApplicationPackage(
	candidates: readonly StrapiPackageCandidate[],
): StrapiPackageCandidate {
	const scored = candidates
		.map((candidate) => ({
			candidate,
			score:
				(candidate.name?.toLowerCase().includes('strapi') ? 1 : 0) +
	...
	if (!scored[0]) throw new Error('Strapi authentic nested web UI package is absent');
	if (scored[1]?.score === scored[0].score)
		throw new Error('Strapi authentic nested web UI package is ambiguous');
...
const assertUrl = (url: string, kind: 'api' | 'archive'): void => {
	...
	if (
		...
		fail(
			...
			expectedHost,
			parsed.host,
			'Strapi request escaped literal consent',
		);
};
...
async function getExact(
		...
		try {
			return await new Promise<Buffer>((resolvePromise, reject) => {
				...
				const call = request(
					url,
					{
						...
						headers: {
							...
							'user-agent': 'versionless-t687',
							'x-versionless-consent-id': STRAPI_CONSENT,
						},
					},
					(response) => {
						observed = true;
						...
						if (response.statusCode !== 200) {
							...
							reject(
								new AdmissionFailure(
									...
									'Strapi response status differs',
								),
							);
							return;
						}
						...
						if (encoding !== 'identity') {
							...
							reject(
								new AdmissionFailure(
									...
									'identity',
									'non-identity',
									'Strapi content encoding differs',
								),
							);
							return;
						}
				...
				call.setTimeout(60_000, () =>
					call.destroy(new Error('Strapi zero-observation timeout')),
				);
	...
	throw new Error('Strapi transport exhausted');
}
...
async function writeTerminal(error: unknown, state: StrapiNetworkState): Promise<void> {
	...
	const failure =
		error instanceof AdmissionFailure
			...
			: new AdmissionFailure(
					...
					'Strapi unexpected acquisition failure',
				);
	const unsigned = {
		schemaVersion: 'versionless.react-strapi-t687-terminal.v1',
		result: 'terminal-failure',
		consentId: STRAPI_CONSENT,
		namespace: STRAPI_NAMESPACE,
		promotion: 'none',
		failure: {
			stage: failure.stage,
...
export async function acquireStrapi(): Promise<void> {
	for (const target of [attemptPath, terminalPath, receiptPath, stageRoot, cacheRoot])
		if (await exists(target)) throw new Error('Strapi T687 output collision');
	await mkdir(evidenceRoot, { recursive: true });
	await writeFile(
		attemptPath,
		`${canonicalize({ schemaVersion: 'versionless.react-strapi-t687-attempt.v1', consentId: STRAPI_CONSENT, namespace: STRAPI_NAMESPACE, limits: { refsPerPrefix: 256, apiBytes: apiCap, archiveBytes: archiveCap, aggregateBytes: aggregateCap } })}\n`,
		{ flag: 'wx' },
	);
	const state: StrapiNetworkState = {
		acceptedResponses: 0,
		aggregateBytes: 0,
		observations: [],
		ledger: [],
	...
	try {
		...
		const apiRoot = joinURL('https://api.github.com', 'repos', 'strapi', 'strapi');
		const repository = JSON.parse((await getExact(apiRoot, 'api', state)).toString('utf8')) as {
			full_name?: unknown;
		};
		if (
			typeof repository.full_name !== 'string' ||
			repository.full_name.toLowerCase() !== 'strapi/strapi'
		)
			fail(
				'repository-identity',
				'canonical-full-name',
				...
				'Strapi repository identity differs',
			);
		const ref = JSON.parse(
			(
				await getExact(joinURL(apiRoot, 'git', 'ref', 'tags', 'v3.6.8'), 'api', state)
			).toString('utf8'),
		) as { ref?: unknown; object?: { type?: unknown; sha?: unknown } };
		if (
			ref.ref !== 'refs/tags/v3.6.8' ||
		...
		if (canonicalize(treeFiles) !== canonicalize(manifestTreeRows))
			fail(
				...
				'Strapi archive differs from the exact untruncated Git tree',
			);
		...
		if (packagePaths.length === 0 || packagePaths.length > 512)
			fail(
				...
				'1..512',
				packagePaths.length,
				'Strapi recursive package inventory differs',
			);
		const documents = new Map<string, PackageDocument>();
		const packageCandidates: StrapiPackageCandidate[] = [];
		...
		for (const packagePath of packagePaths) {
			const document = parseStrapiJsonc(
			...
			packageCandidates.push({
				...
				search:
					productCapabilityCorpus.includes('content-manager') ||
					(productCapabilityCorpus.includes('content') &&
						productCapabilityCorpus.includes('collection')),
				detail:
					productCapabilityCorpus.includes('publish') ||
					(productCapabilityCorpus.includes('edit') &&
						productCapabilityCorpus.includes('entry')),
				auth: corpus.includes('login') || corpus.includes('auth'),
				registryApi:
					(corpus.includes('request') || corpus.includes('api')) &&
					(corpus.includes('save') ||
						corpus.includes('create') ||
						corpus.includes('update')),
			});
		}
		const selectedPackage = (() => {
			try {
				return discoverStrapiApplicationPackage(packageCandidates);
			} catch (error) {
				return fail(
					'semantic-qualification',
					...
					packageCandidates,
					error instanceof Error ? error.message : 'Strapi package discovery failed',
				);
			}
		})();
		...
		const properties = [
			{
				...
				expected: 'Strapi application',
				observed: packageDocument.name,
				pass:
					typeof packageDocument.name === 'string' &&
					packageDocument.name.toLowerCase().includes('strapi') &&
			...
			{
				property: 'content-manager-list-create-capability',
				expected: 'content-manager list/create capability',
				observed:
					(corpus.includes('content-manager') || corpus.includes('collection')) &&
					(corpus.includes('create') || corpus.includes('newentry')),
				pass:
					(corpus.includes('content-manager') || corpus.includes('collection')) &&
					(corpus.includes('create') || corpus.includes('newentry')),
			},
			{
				property: 'content-open-edit-publish-capability',
				expected: 'open/edit/publish entry capability',
				observed:
					corpus.includes('edit') &&
					(corpus.includes('entry') || corpus.includes('document')) &&
		...
		for (const property of properties)
			if (!property.pass)
				fail(
					...
					property.expected,
					property.observed,
					`Strapi semantic property failed: ${property.property}`,
				);
		const unsigned = {
			schemaVersion: 'versionless.react-strapi-ingest.v1',
...
export async function verifyStrapiIngest(): Promise<{
	valid: true;
	digest: string;
	files: number;
}> {
	...
	if (canonicalize(receipt) !== canonicalize(cached))
		throw new Error('Strapi receipt publication differs');
	const { integrity, ...unsigned } = receipt;
	if (integrity.canonicalDigest !== sha256(canonicalize(unsigned)))
		throw new Error('Strapi receipt integrity differs');
	...
	if (
		...
		throw new Error('Strapi offline source replay differs');
	return { valid: true, digest: integrity.canonicalDigest, files: rows.length };
}

export async function main(args = process.argv.slice(2)): Promise<void> {
	const mode = parseStrapiLauncher(args);
	if (mode === 'preflight') {
		process.stdout.write('{"preflight":"pass"}\n');
		return;
	}
	if (mode === 'acquire') await acquireStrapi();
	process.stdout.write(`${canonicalize(await verifyStrapiIngest())}\n`);
}
if (basename(process.argv[1] ?? '') === 'react-strapi-ingest.ts')
