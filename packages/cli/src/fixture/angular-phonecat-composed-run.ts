...
const fixtureFile = path.join(root, 'fixtures/angular-phonecat-composed/fixture.json');
...
const workRoot = path.join(root, '.versionless/work/angular-phonecat-composed');
const artifactsRoot = path.join(root, 'evidence/runs/angular-phonecat-composed/artifacts');
const guard = path.join(root, 'packages/node-guard/dist/index.cjs');
const expectedJourneySha256 = '299308578fc043f3d09d3e189c1e14a9b1d12d4f42df37dfbf89bb9c4c2e1300';
...
const intendedMutationFailure = createRegExp(
	anyOf(
...
async function runJourney(
	...
	try {
		...
		if (
			...
			intendedMutationFailure.test(error.message)
		)
...
function outputDigest(sources: Awaited<ReturnType<typeof sourceSlice>>): string {
	return sha256(
		canonicalize(
			Object.fromEntries(
...
async function mutationProof(
	...
	const first = restored.indexOf(before);
	if (first < 0 || restored.indexOf(before, first + 1) >= 0)
		throw new Error(`Exact ${seam} mutation target missing or ambiguous`);
	const mutated = `${restored.slice(0, first)}${after}${restored.slice(first + before.length)}`;
	let failure;
	try {
	...
	if (!failure || sha256(await readFile(file)) !== restoredSha256)
		throw new Error(`${seam} restoration was not byte-identical`);
...
export async function verifyAngularPhonecatComposed({
	receiptPath,
}: {
	receiptPath: string;
	...
	if (
		...
		throw new Error('Pinned PhoneCat composition input mismatch');
	await verifyReceipt(path.join(root, 'evidence/runs/angular-phonecat/t014-run.json'));
	await verifyReceipt(
		path.join(root, 'evidence/runs/angular-phonecat-route-resolve/t032-run.json'),
	);

	await rm(workRoot, { recursive: true, force: true });
	const legacy = path.join(workRoot, 'legacy');
	const target = path.join(workRoot, 'target');
	const reverse = path.join(workRoot, 'route-then-lexical');
	await mkdir(workRoot, { recursive: true });
	await Promise.all(
		[legacy, target, reverse].map((directory) =>
			cp(sourceRoot, directory, { recursive: true }),
		),
	);
	const pristine = await sourceSlice(sourceRoot);
	const lexicalFirst = transformPhoneDetailLexicalThis(pristine.phoneDetail);
	const lexicalThenRoute = transformPhoneRouteResolve({
		...pristine,
		phoneDetail: lexicalFirst.code,
	});
	const routeFirst = transformPhoneRouteResolve(pristine);
	const routeThenLexical = transformPhoneDetailLexicalThis(routeFirst.code.phoneDetail);
	const reverseOutput = { ...routeFirst.code, phoneDetail: routeThenLexical.code };
	if (canonicalize(lexicalThenRoute.code) !== canonicalize(reverseOutput))
		throw new Error('PhoneCat composition order produced different outputs');
	await Promise.all([
		writeSources(target, lexicalThenRoute.code),
		writeSources(reverse, reverseOutput),
	]);
	const firstOutputSha256 = outputDigest(await sourceSlice(target));
	const secondOutputSha256 = outputDigest(await sourceSlice(reverse));
	if (firstOutputSha256 !== secondOutputSha256)
		throw new Error('PhoneCat composition output digest differs by order');
	for (const key of Object.keys(applicationFiles) as Array<keyof typeof applicationFiles>) {
		const sourceHash = sha256(pristine[key]);
		const targetHash = sha256(lexicalThenRoute.code[key]);
		if (sourceHash === targetHash)
			throw new Error(`Expected composed file did not change: ${applicationFiles[key]}`);
	}
	...
	if (outputDigest(await sourceSlice(target)) !== firstOutputSha256)
		throw new Error('Mutation effects leaked into the composed target');
		...
		orders: [
			{
				...
				outputSha256: firstOutputSha256,
				preconditions: [lexicalFirst.preconditions, lexicalThenRoute.preconditions],
			},
			{
				order: ['phone-route-resolve-component-binding', 'phone-detail-lexical-this'],
				outputSha256: secondOutputSha256,
				preconditions: [routeFirst.preconditions, routeThenLexical.preconditions],
			},
		],
	...
	const journeyArtifact = await artifact('journey.json', journeys);
	if (journeyArtifact.sha256 !== expectedJourneySha256)
		throw new Error('Composed PhoneCat journey digest changed');
	const artifacts = [
		await artifact('preparation.json', preparation),
		...
		await artifact('composition.json', composition),
		await artifact('mutation.json', { mutations }),
		await artifact('migration-diff.json', {
			changedFiles,
			files: lexicalThenRoute.files.map((file) => ({
				path: file.path,
				sourceSha256: sha256(pristine[file.key]),
				targetSha256: file.targetSha256,
				edits: file.edits.length,
			})),
		}),
		await artifact('transform.json', {
			semanticEngine: lexicalThenRoute.semanticEngine,
	...
	const receipt: MigrationReceipt = {
		...
		runId: 'T048-angular-phonecat-composed',
		fixture: manifest.id,
		source: {
