...
versionless is a toolkit for exactly that app. You point one command at it, and versionless:

1. **Reads** the app — what framework, what version, what builder, what era of Node it expects.
2. **Rewrites** it into a separate folder (called a *lane*). It never, ever writes into your original app. Not one byte.
3. **Proves** the result — it installs, builds, and then drives *both* the old app and the new one through the same journeys in a real browser, and checks that they behave the same.
...
If it hits something it doesn't understand — a framework it has no adapter for, a lockfile from a different package manager, a dependency it has never verified — it stops and tells you *by name* what it can't do:

```
refused: install.lockfile-foreign
```

That's not a crash. That's the tool being honest. Every refusal has a name, every name is counted, and the list of things it *won't* claim is published right next to the list of things it will. (If you've ever been burned by a tool that "succeeded" and quietly broke your app... yeah. That's the failure mode this design exists to prevent.)
...
Every command takes `--help` and `--json`. Start with `analyze` — it can't hurt anything, and it'll tell you immediately whether versionless recognizes your app.
...
The two documents that carry the whole picture (both generated, never hand-edited — an edited copy fails verification instead of changing a claim):
...
- [`evidence/trust/current/enterprise-report.md`](evidence/trust/current/enterprise-report.md) — the full evidence report: sources, versions, hashes, journeys, results, and the claims-and-non-claims one-pager

And to be honest about where things stand today: the React line runs end to end (several real apps have gone command → migrated → browser-proven with zero human intervention). The Angular line reaches the build stage and stops on a small set of known, named defects that are queued for a fix. The coverage report says exactly which apps, exactly which stages — go look, it's the source of truth.

## A few more things it never does

- **Never touches your app.** All writes go to the lane. The tool counts its own interventions per run; a migration only counts as *proven* if that count is zero.
