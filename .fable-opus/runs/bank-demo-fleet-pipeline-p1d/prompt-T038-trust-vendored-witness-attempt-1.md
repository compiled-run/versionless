...
State you inherit (T037): the clean-worktree gate PASSES (`interventionCount 0`, `proven`, idempotent, witness digest identical) _because_ `@async/witness` is now vendored (`package.json`: `"file:vendor/async-witness-0.8.0.tgz"`; `vendor/README.md` records source `../witness` @ `83b86de`). But the main tree is RED — `trust:verify` throws and `npm test` reads 31 failed / 2675 passed — from exactly three collisions, all named:
